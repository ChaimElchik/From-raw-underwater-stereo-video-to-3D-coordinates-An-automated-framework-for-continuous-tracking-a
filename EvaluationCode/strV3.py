...
def run_geometric_matching(file1_path, file2_path, mat_path):
    ...
    # We remove the filter inside the loop to capture "guesses" for difficult IDs.
    ...
    # Prioritize: 
    #   1. "Valid" matches (dist < 50)
    #   2. High Frequency (count)
    #   3. Low Error (avg_dist)
    
    consensus['is_valid'] = consensus['avg_dist'] < 50.0
    
    consensus = consensus.sort_values(
        by=['is_valid', 'count', 'avg_dist'], 
    ...
    for _, row in consensus.iterrows():
        ...
        # If both IDs are still available, take the match
...
if __name__ == "__main__":
    ...
    print("Final ID Mapping (Resolved Underwater Conflicts):")
    print(mapping[['id1', 'id2', 'count', 'avg_dist']].sort_values('id1'))
    
    renamed_df.to_csv('406__synchronized.csv', index=False)